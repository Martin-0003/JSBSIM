...
def process_csv_folders(folder_paths):
    ...
    raiz_sigma_3300ft = ((1 - 22.558 * 10**-6 * 3300) ** 4.2559) ** (1 / 2)
    Tas_Eas_1700ft = fts_a_ms * raiz_sigma_1700ft
    Tas_Eas_3300ft = fts_a_ms * raiz_sigma_3300ft
    ...
    for folder_path in folder_paths:
        ...
        # Convert units
        if "b" in result_df['NumericPart']:
            result_df['NumericPart'] *= Tas_Eas_3300ft  # Convert to m/s EAS
        else:
            result_df['NumericPart'] *= Tas_Eas_1700ft  # Convert to m/s EAS
        
        result_df['LastValue'] *= lbf_a_N  # Convert to N
...
def plot_data(folder_paths, output_file, graph_file):
    ...
    plt.ylabel('Drag (N)')
    plt.title('Drag/Speed Comparison')
    plt.legend(loc='best')
